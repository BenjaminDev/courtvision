...
#show: ieee.with(
  ...
  abstract: [
    Extracting useful information from single viewpoint videos of padel games relies on relating objects with frame to a world coordinate system. The estimation of ball and player world coordinates using a single camera is an inherently ill posed problem. The measurement space is 2D in pixel units and the state space is 3D in meters. This paper explores the use of prior knowledge about the scene (court geometry) and object dynamics to produce world coordinate estimates. Additionally, a technique for single image camera calibration is presented that exploits the known scene geometry. Evaluation is done on a new dataset of live broadcast footage from the _Ooredoo Qatar Major 2023_ tournament. The dataset as well as code is made available to the community as #bluelink("https://benjamindev.github.io/courtvision/", "CourtVision").
...
\
Related work @LiuPaul which uses a single camera to track 3D trajectories of a shuttle cock during badminton games used a number of domain specific constraints. There seeded the tracker with initial positions and velocity constraints. This work took the view to shift prior knowledge into a distribution and apply a bayesian filtering process. By placing such constraints into prior distributions the proposed tracker aims to easily generalizable to other racket sports. The implementation of this bayesian filtering process is realized as a particle filter.
