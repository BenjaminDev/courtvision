...
Looking at the player positions during rallies and games is a useful visualization. It gives insight into player tactics and movement. The player positions are projected onto the court floor and a heatmap is generated. The heatmap is generated by summing the player positions over all frames for a rally in @rally_heatmap and a game @game_heatmap.

#figure(
  image("images/rally_heatmap.png", width: 100%, height: 6cm),
...
= Discussion
This work tackled a number of common vision problems and applied them to the domain of padel. The results are encouraging and show promise for single camera systems.  The single image camera calibration technique is robust and can be used to calibrate any camera with a known scene geometry.
= Conclusion
