...
As part of this project a dataset *CourtVisionPadelDataset* was compiled from videos from the _Ooredoo Qatar Major 2023_ tournament. This dataset consists of 6 hours of live broadcast. It's partially segmented to give timestamps of rallies and bounding boxes of the ball. Accompanying the dataset is a Python library, *CourtVision*, that implements the techniques described in this paper. #bluelink("https://benjamindev.github.io/courtvision/","CourtVision") uses _Kornia_ @Riba2019 a _Pytorch_ @pytorch based computer vision library.
...
The _predict_ phase incorporates dynamics of the ball $p(x#sub[t]|x#sub[t-1])$ which propagates each particle according to the following pseudo code:
...
The ball tracking converges to a smooth trajectory when consecutive detections are made. @ball_full_state shows the internal representation of the tracker for the ball. At each time step the tracker holds a full posterior distribution of the state given all previous states and observations. The shape of this distribution changes and is able to quickly adapt when the ball bounces or hits a wall. This allows for consistent tracking despite not having impacts (hits and bounces) included in the model. The mean estimate of the ball position is shown in @ball_mean_estimate. The tracker is able to handle false detections but too many will cause the tracker to fail. The ball detector is not very robust and false detections are common. @tracker_false_detections shows the trackers' internal state when false detections are present. The tracker is able to recover from these false detections and continue tracking the ball.
#figure(
  image("images/Ball_mean_estimate.png", width: 100%),
  caption: [Showing the mean estimate of the ball position.],
) <ball_mean_estimate>

#figure(
  image("images/Ball_full_state.png", width: 100%),
  caption: [Showing the full state of the tracker for the ball.],
) <ball_full_state>

#figure(
  image("images/tracker_false_detections.png", width: 100%),
  caption: [Lower frame shows the image plane detections with false detections on a players foot and the camera operator outside the court. The upper frame shows the trackers' internal state.],
) <tracker_false_detections>



== Data visualizations <visualizations>
Looking at the player positions during a single, and across multiple, rallies during a game is a useful visualization. It gives insight into player tactics and movement. The player positions are projected onto the court floor. Here player The heatmap is generated by summing the time a player is at a position over all frames for a rally in @rally_heatmap and over multiple rallies in @game_heatmap. Velocity heatmaps are generated in a similar fashion. The velocity is calculated as the difference in position per time interval. The figures come from rally _635b_ and game _0000_ of the dataset @de_Charmoy_CourtVision_2023.
...
These are examples of the visualizations that can be generated from the data. The full set of visualizations are made available @de_Charmoy_CourtVision_2023.
